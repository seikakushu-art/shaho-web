 ...
 * @returns 第2号被保険者に該当する場合true、該当しない場合false
 */
function calculateCareSecondInsured(
  birthDate: Date | string | number | undefined | null,
): boolean {
    ...
    try {
      ...
      records.forEach((record, index) => {
        ...
        const normalizeBoolean = (value?: boolean | string): boolean | undefined => {
          ...
          if (typeof value === "boolean") return value;
          const str = String(value).trim().toLowerCase();
          // 空文字列の場合はundefinedを返す（生年月日からの自動判定を可能にするため）
        ...
        const normalized: ShahoEmployee = {
          ...
          // 介護保険第2号被保険者フラグは、APIから送信された値があれば優先的に使用
          // 値がない場合のみ生年月日から自動判定
          careSecondInsured: (() => {
            const apiValue = normalizeBoolean(record.careSecondInsured);
            if (apiValue !== undefined) {
              return apiValue;
            }
            // APIから値が送信されていない場合のみ、生年月日から自動判定
