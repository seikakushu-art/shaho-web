      ...
      const result = await this.approvalWorkflowService.startApprovalProcess({
        ...
        onApproved: async () => {
          ...
          const { basicInfo, socialInsurance, dependentInfo, dependentInfos } =
            approvedRequest.employeeData;
          const healthStandardMonthly =
            socialInsurance.healthStandardMonthly ?? 0;
          const welfareStandardMonthly =
            socialInsurance.welfareStandardMonthly ?? 0;
          ...
          const employeeData: ShahoEmployee = {
            ...
            currentLeaveStartDate:
              socialInsurance.currentLeaveStartDate || undefined,
            currentLeaveEndDate:
              socialInsurance.currentLeaveEndDate || undefined,
            exemption: socialInsurance.exemption || false,
          };

          // undefinedのフィールドを除外
          const cleanedData = this.removeUndefinedFields(
            employeeData as unknown as Record<string, unknown>,
          );

          // 編集モードかどうかを判定
          const isEdit = this.isEditMode && this.employeeId;
          const updateData: Partial<ShahoEmployee> =
            cleanedData as unknown as Partial<ShahoEmployee>;
            ...
            if (approvalHistories.length > 0) {
              ...
              if (latestHistory.actorId) {
                try {
                  ...
                  approverDisplayName =
                    approver?.displayName || latestHistory.actorId;
                } catch (error) {
          ...
          if (isEdit && this.employeeId) {
            ...
            this.approvalMessage =
              '承認が完了しました。社員データを更新しました。';
          } else {
            // 新規登録モード：新規社員を追加
            const result = await this.employeesService.addEmployee(
              cleanedData as unknown as ShahoEmployee,
            );
          ...
          console.log(
            ...
            JSON.stringify(dependentInfosToSave, null, 2),
          );
          console.log(
            'dependentInfos:',
            JSON.stringify(dependentInfos, null, 2),
          );
          ...
          if (employeeId) {
            ...
            const shouldSkipDependentUpdates =
              basicInfo.hasDependent === undefined &&
              dependentInfosToSave.length === 0;

            console.log('扶養情報の更新判定', {
              shouldDeleteExistingDependents,
              shouldSkipDependentUpdates,
              filteredDependentsCount: filteredDependents.length,
            });

            if (shouldSkipDependentUpdates) {
              console.log(
                'hasDependent未指定かつ扶養データなしのため、既存の扶養情報は変更しません。',
              );
            } else {
              // hasDependent=false もしくは新しい扶養データがある場合のみ既存扶養を削除
              if (shouldDeleteExistingDependents) {
                ...
                console.log('既存の扶養情報は削除しません。');
              }

              if (basicInfo.hasDependent === false) {
                console.log(
                  'hasDependent=false のため、新規扶養情報の追加は行いません。',
                );
              } else if (filteredDependents.length > 0) {
                console.log(
                  '新しい扶養情報を保存します。件数:',
                  filteredDependents.length,
                );
                for (const depInfo of filteredDependents) {
                  ...
                  console.log(
                    '保存する扶養情報:',
                    JSON.stringify(dependentData, null, 2),
                  );
                ...
                // hasDependent=true かつ空配列など、既存扶養を残したまま何もしないパス
                console.log('保存対象の扶養情報がありません。');
              }
            }
          } else {
