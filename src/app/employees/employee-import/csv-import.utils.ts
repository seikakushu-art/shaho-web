...
export function normalizeEmployeeNoForComparison(employeeNo: string): string {
  ...
  export function validateRequiredFields(
    ...
    if (employeeNo && allRows) {
      ...
      const sameEmployeeRows = allRows.filter((r) => {
        ...
        return rEmployeeNo && normalizeEmployeeNoForComparison(rEmployeeNo) === normalizedEmployeeNo;
      });
      
      if (sameEmployeeRows.length > 1) {
        // 同じ社員番号で複数行ある場合、現在の行が最初の行以外かどうかを判定
        const sortedRows = sameEmployeeRows.sort((a, b) => a.rowIndex - b.rowIndex);
  ...
  export function validateBusinessRules(row: ParsedRow, templateType: TemplateType): ValidationError[] {
    ...
    if (hasDependentFlag) {
      ...
      const isNoDependent = ['0', 'false', 'no', 'off', '無'].includes(normalizedFlag);
      ...
      if (isNoDependent) {
        ...
        if (hasDependentInfo) {
          errors.push(buildError(row.rowIndex, '扶養の有無', '扶養の有無が「無」の場合、扶養家族情報は入力できません', templateType));
        }
      }
    }
  ...
  export function validateFileLevelRules(rows: ParsedRow[], templateType: TemplateType): ValidationError[] {
    ...
    employeeNos.forEach((rowIndices, employeeNo) => {
      ...
      if (firstRow) {
        ...
        if (hasDependentFlag) {
          const normalizedFlag = hasDependentFlag.trim().toLowerCase();
          isNoDependent = ['0', 'false', 'no', 'off', '無'].includes(normalizedFlag);
        }
  ...
  export function calculateDifferences(
    ...
    if (isDependentOnlyRow) {
      ...
      if (foundByEmployeeNo && foundByEmployeeNo.dependents && csvDependentNameKanji) {
        // 扶養 氏名(漢字)で既存扶養家族を検索（スペースを無視して比較）
        const normalizedCsvName = normalizeNameForComparison(csvDependentNameKanji);
        existingDependent = foundByEmployeeNo.dependents.find((dep) => {
          if (!dep.nameKanji) return false;
          const normalizedExistingName = normalizeNameForComparison(dep.nameKanji);
    ...
    if (isUpdate && existingEmployee) {
      ...
      if (csvDependentNameKanji && existingEmployee?.dependents) {
        const normalizedCsvName = normalizeNameForComparison(csvDependentNameKanji);
        existingDependent = existingEmployee.dependents.find((dep) => {
          if (!dep.nameKanji) return false;
          const normalizedExistingName = normalizeNameForComparison(dep.nameKanji);
          return normalizedExistingName === normalizedCsvName;
        }) || null;
